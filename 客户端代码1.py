...
def chatClient(clientsocket):
    ...
    if str(extension) in ('.py','.txt','.doc'):
        ...
        clientsocket.send(("file"+","+str(data3)).encode("utf-8"))
        if str(data2)!="b''":
        ...
        clientsocket.send(("photo"+","+str(data)).encode("utf-8"))
        img = cv2.imread(data)
        ...
        if str(data)!="b''":
            for i in range(len(data)//1024+1):
                if 1024*(i+1)>len(data):
                    clientsocket.send(data[1024*i:])
                    print('第'+str(i)+'次发送数据')
                else:
                    clientsocket.send(data[1024*i:1024*(i+1)])
                    print('第'+str(i)+'次发送数据')
        ...
        clientsocket.sendto(("vedio" + "," + "{}".format(fname + extension) + "," + str(size[0]) + "," + str(size[1])).encode(),("127.0.0.1", 8000))
        while True:
            ...
            if flag == True:
                ...
                fhead = struct.pack('l', len(data))
                # 发送文件头、数据:
                clientsocket.sendto(fhead, ('127.0.0.1', 8000))
                for i in range(len(data) // 1024 + 1):
                    if 1024 * (i + 1) > len(data):
                        clientsocket.sendto(data[1024 * i:], ('127.0.0.1', 8000))
                    else:
                        clientsocket.sendto(data[1024 * i:1024 * (i + 1)], ('127.0.0.1', 8000))
                    print('第' + str(i) + '次发送数据')
            else:
                clientsocket.sendto('END'.encode('utf-8'), ('127.0.0.1', 8000))
                break
    else:
    ...
    while True:
        ...
        if receive[0]=='file':
            ...
            if str(receive[1].encode('utf-8'))!="b'end'":
                ...
                f.close()
                listbox.insert(tk.INSERT,'系统提醒：服务器发来的文件已另存为' + filename + '\n','red')
                listbox.see(END)
            else:
                continue
            ...
            if str(receive[1].encode('utf-8'))!="b'end'":
                ...
                buf=clientsocket.recv(fhead_size)
                if buf:
                ...
                cv2.imwrite('client.png',img_decode)

                jieshou= tk.PhotoImage(file='./client.png')
                listbox.insert(tk.INSERT,'系统提醒：客户端发来的图片已另存为client.png \n','red')
            ...
            while 1:
                receive, addr = clientsocket.recvfrom(1024)
            ...
            listbox.see(END)
def leave(clientsocket):
    root.destroy()
def openfile(clientsocket):
    text.delete('0.0','end')
...
# 开始截图
def buttonCaptureClick(clientsocket):
    # 最小化主窗口
    #root.state('icon')
    #sleep(0.2)
    filename = 'temp.png'
...
def mark(exp):  # 参数是发的表情图标记, 发送后将按钮销毁
    global ee
    #clientsocket.send(("emoji"+","+str(exp)).encode("utf-8"))
    clientsocket.send(("emoji" + "," + str(exp)).encode("utf-8"))
...
def sendmail(clientsocket):
    window=tk.Toplevel()
    window.geometry("250x300")#定义窗口尺寸本例定义大小
    ...
    def send():
        # 打印输入框信息
        # 发件人
        user = '644972559qq.com'
        # 收件人
        to = to_input.get()
        # 密码
        pwd = 'bnvnhaqwaxkabfhj'
        # 邮箱内容
...
photo2 = tk.PhotoImage(file='./FTP.png')
ftp= tk.Label(root,image=photo2,bg = "white")

ftp.grid(row = 3,column=6)#调整位置#设置主界面
...
b1=tk.Button(root,text="发送",bg = "white",command=lambda:chatClient(clientsocket))
b1.grid(row=5,column=4)
b2=tk.Button(root,text="关闭",bg = "white",command=lambda:leave(clientsocket))
b2.grid(row=5,column=5)
b3=tk.Button(root,text="打开文件",bg = "white",image=pfile,command=lambda:openfile(clientsocket))
b3.grid(row=3,column=2)
b4=tk.Button(root,text="打开图片",bg = "white",image=pphoto,command=lambda:openpic(clientsocket))
b4.grid(row=3,column=3)
b5=tk.Button(root,text="截屏",bg = "white",image=pcut,command=lambda:buttonCaptureClick(clientsocket))
b5.grid(row=3,column=4)
b6=tk.Button(root,text="表情",bg = "white",image=pemoji,command=lambda:express(clientsocket))
b6.grid(row=3,column=1)
b6=tk.Button(root,text="邮件",bg = "white",image=pemail,command=lambda:mail())
...
t = threading.Thread(target=receivedata,args=(clientsocket,))
t.start()
tk.mainloop()
